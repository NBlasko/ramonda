 ...
 * because three browsers are not something a local gate should download.
 *
 * **CI pays it**, in the `browser` job — which already installs an engine and is deliberately off
...
const wrote = writeOrCheck(
  file,
  `// Generated by scripts/build-shorthand-leaves.mjs from mdn-data (CC0-1.0) and from Chromium,\n// Firefox and WebKit, MEASURED rather than read: each browser is launched and asked what it has.\n// No engine source is used. See THIRD-PARTY.md. Do not edit.\n` +
    `//\n` +
...
console.log(`[leaves] ${wrote ? "wrote" : "up to date —"} ${sorted.length} shorthands`);
