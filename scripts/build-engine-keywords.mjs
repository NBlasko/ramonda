...
const names = Object.keys(extra).sort();
const total = names.reduce((n, one) => n + extra[one].size, 0);
writeFileSync(
  OUT,
  `// Generated by scripts/build-engine-keywords.mjs. Do not edit.\n` +
    ...
    `//\n` +
    counts.map(([engine, n]) => `// ${engine.padEnd(10)} ${String(n).padStart(4)}\n`).join("") +
    `// ${"the union".padEnd(10)} ${String(total).padStart(4)} across ${names.length} properties\n` +
    `\n` +
    ...
    names.map((one) => `  ${JSON.stringify(one)}: ${JSON.stringify([...extra[one]].sort())},\n`).join("") +
    `};\n`,
);

for (const [engine, n] of counts) console.log(`[keywords] ${engine.padEnd(10)} ${String(n).padStart(4)}`);
console.log(`[keywords] wrote ${total} value(s) across ${names.length} properties`);
