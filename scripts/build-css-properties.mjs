...
/** The same, for the two whose remaining identifier is a property name — see `NAMES_A_PROPERTY`. */
const propertyNamedRows = [];
for (const name of named) {
  if (!NAMES_A_PROPERTY.has(name)) continue;
  const scanned = scan(name);
  propertyNamedRows.push(`  ${JSON.stringify(name)}: ${JSON.stringify(scanned.words.join(" "))},`);
}
...
const said =
  ...
  `${propertyNamedRows.length} whose value is a property name`;
