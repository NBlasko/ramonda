 ...
 * convention exists the convention is the POINT rather than the source: a reader who has met `p-4`
 * anywhere should read `r-p-4px` without being told. A property with no entry uses its own name,
 * which is already readable: `outline-offset: 4px` becomes `r-outline-offset-4px`.
 *
 * **Twenty of the sixty-five match TAILWIND's spelling**, and it is named rather than left as "a
 * convention", because two of them are its words rather than anyone's abbreviation: `rounded` for
 * `border-radius`, and `px`/`py`, which Tailwind uses for the horizontal and vertical axes while
 * these are the INLINE and BLOCK ones. The rest — `p`, `m`, `w`, `h`, `bg` and the `pt`/`ml` family —
 * are the first letters any CSS tool would reach for.
 *
 * Nothing is copied: these are single words in a table written here, and Tailwind is MIT in any
 * case. Saying whose spelling it is costs nothing and is the honest version of "a convention".
 *
...
const units = `// Generated by scripts/build-css-properties.mjs from mdn-data (CC0-1.0). Do not edit.
//
