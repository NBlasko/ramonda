...
`interval-with-no-cleanup`'s advice names the `Timer` hook for an interval the app starts.
...
`Timer` is the answer for that case now, so the advice offers it first and keeps the property
