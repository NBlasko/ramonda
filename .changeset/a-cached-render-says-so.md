...
> `<Board />` has a cached render, so RMD020 cannot see inside it — an inline handler, an object rebuilt in
> place and a value that does not come from state all go unreported here. A cached render also refreshes
> only when a SIGNAL it read moves, so anything else it reads keeps its old value. Both are the deal;
