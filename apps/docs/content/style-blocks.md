...
`rules` takes a rule's id, and a typo is caught: writing `unknown-unti` tells you so and names the
one you meant. These are the ids.

[css-rules:start]: # "generated by scripts/build-rule-tables.mjs — edit the lines there"

Every one of them fails the build. 32 of them, and each is a key you can switch off.
...
Every message you meet begins with its rule's id, so the one you want to silence is the one the
