...
| A `params()` read claiming a key the routing does not supply | `params<{ teamId: string }>()`, a plain destructuring, or `params().teamId` under a route with no `:teamId` — see [reading the URL](/routing/params#params-is-judged-by-the-keys-it-names) |
...
`findings`, and it is the name the command prints. The table below is **generated from the rules**,
so a rule cannot be added without appearing here.

[rules:start]: # "generated by scripts/build-rule-tables.mjs — edit the rule, not this"

**Every one of them fails the run.** 86 rules.
...
### Every rule fails the run, and how to say "not here"

There is no warning level. A rule reports a fault or it does not, and a reported fault fails the
command — because a warning that never fails anything is read once and then not read at all, while
a refusal is dealt with.

Which only works because there is somewhere to put "I know, and it is right here":
...
**It records rather than silences.** Every annotated site is listed on every run, with the words
its author wrote:
...
unnoticed. It is a decision somebody made and can be asked about, which a warning nobody looked at
is not. **Line-scoped, never file-scoped** — one line blinding a whole file is exactly what
somebody in a hurry reaches for.

### The bar a rule has to clear before it ships

Every rule above was measured against every app and package in the Ramonda repository when it was
written, and **all but one report zero.** That is deliberately hard to clear: a rule that already
has something to say about correct code is not ready to be written.
...
[`RMD051`](/reference/diagnostics/rmd051) exists to say. Where that inference is the right answer,
the directive above says so in one line and the run stays green.
