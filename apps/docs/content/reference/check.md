...
**Errors.** These fail the run. 3 of them.
...
**Warnings.** These print and the run still passes. 25 of them.
...
| `unguarded-async-lifecycle` | an `async` lifecycle awaits something with no `try` or `.catch` to handle a failure — also [`RMD057`](/reference/diagnostics) |
