...
| `Timer` | A timer the app arms and the framework clears: `after(ms, run)`, `repeat(ms, run)`, `stop()`. One instance is one timer; arming returns whether it armed, and teardown clears it. [Timers](/concepts/timers#a-timer-that-starts-when-you-say) |
...
`IntervalWithNoCleanupIssue`, `LateRequestReadIssue`, `LinkWithoutADestinationIssue`,
`ListenerOnTheDefaultHostIssue`, `MediaWithNoCaptionsIssue`, `NamedOnlyByAPlaceholderIssue`,
