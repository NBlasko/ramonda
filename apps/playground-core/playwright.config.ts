 ...
 * PERSON does to it: a caret that the platform moves when a value is assigned, focus that a node
 * loses when it is picked up and put down, a `<select>` deciding which option is showing. Every one
 * of those was measured in jsdom, agreed with, and then needed a browser to be believed — and one
 * of them was wrong in the browser for a reason jsdom could never have shown (a button takes the
 * focus when you press it, so there was nothing left in the list to keep).
 *
...
export default defineConfig({
  testDir: "./browser",
  // One browser. The claims here are about the platform's own behaviour — caret, focus, selection —
  // which is the same across engines; a matrix would buy repetition rather than coverage.
