...
export function namesIn(declarations: Declarations): readonly Named[] {
  const found: Named[] = [];
  /** Which path claimed a name, so a clash can name BOTH rather than only the loser. */
  ...
  const walk = (group: Declarations, trail: readonly string[]): void => {
    for (const [segment, one] of Object.entries(group)) {
      ...
      if (isVariable(one)) {
        ...
        /**
         * Two paths can spell one name, and the config cannot see it happening.
         *
         * `{ "a-b": { c } }` and `{ a: { "b-c" } }` both reach `--a-b-c`, because the separator
         * between levels is a character a name may also contain. Left alone the second would
         * overwrite the first in the stylesheet and `$` would offer two paths to one variable, with
         * nothing said anywhere.
         */
        if (already !== undefined) {
          refuse(
            `two variables spell one custom property.\n\n` +
              `        \`${already}\` and \`${here.join(".")}\` both become \`${name}\`.\n\n` +
              `        A level is joined to the next with a dash, and a name may hold one too, so\n` +
              `        two paths can meet. Rename either.`,
          );
        }

        claimed.set(name, here.join("."));
        found.push({ path: here.join("."), name, kind: one.kind, value: one.value as string | number });
        continue;
      }
