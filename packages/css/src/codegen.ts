...
function propertyMap(rules: PropertyRules | undefined, variablesOnly: readonly string[] = []): Mapped {
  const rows: string[] = [];
  ...
  for (const [property, primitive] of Object.entries(PRIMITIVE)) {
     ...
     */
    const onlyVariables = narrow.kinds.some((one) => variablesOnly.includes(one));
...
export function generate(
   ...
   */
  const constrained = Object.keys(rules ?? {}).length > 0 || variablesOnly.length > 0;
   ...
   */
  const { rows, removed, uses, closed } = propertyMap(rules, variablesOnly);
