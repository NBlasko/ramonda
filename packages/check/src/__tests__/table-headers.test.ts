 ...
 * header association is the only thing that lets it say "Price, £4.48" instead of "£4.48", read out
...
describe("a table whose cells have no heading", () => {
  test("a table written out with data rows and no `<th>`", () => {
    // 16 is the flat form; 28 goes through `<thead>`/`<tbody>`, which changes nothing about the
    // association — a `<td>` inside a `<thead>` is still a data cell.
    expect(lines()).toEqual([16, 28]);
  });

  /**
   * Eight silences, and the third of them is the whole shippability of the rule.
   *
   * 42 and 54 write their headers out, by column and by row. 62 builds its rows from data and 71
   ...
   *
   * 79 and 87 are LAYOUT tables and say so; `role="presentation"` is exactly how an author declares
   * that a table is not data, and reporting one would be reporting the documented way of writing
   * the thing this rule does not care about. 94 has no rows and 97 only a caption — scaffolding
  ...
  test("everything this cannot see, and everything that is not data, stays silent", () => {
    for (const quiet of [42, 54, 62, 71, 79, 87, 94, 97]) {
