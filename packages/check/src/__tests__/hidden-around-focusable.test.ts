...
describe("a hidden subtree a keyboard can still tab into", () => {
  test("a control inside, at one level and at two", () => {
    // 17 is the modal shape; 22 is an `<a href>` two elements down, which is how it really looks.
    expect(said()).toContain("17:button");
    expect(said()).toContain("22:a");
  });
  ...
  test("and something put in the tab order by hand, and the bare spelling of the attribute", () => {
    ...
    expect(said()).toContain("30:div");
    expect(said()).toContain("35:input");
  });

  test("the whole list, so a regression cannot go quiet", () => {
    expect(said()).toEqual(["17:button", "22:a", "30:div", "35:input"]);
  });

  /**
   * Nine silences, and the two fixes are the first of them.
   *
   * 40 uses `inert`, which the platform added for exactly this and which does the focus half as
   * well. 45 takes the control out of the tab order by hand, which is the other fix — reporting it
   * would be reporting the fix. 52 has nothing focusable, 58 an `<a>` with no `href` and 63 an
   * `<input type="hidden">`, neither of which a keyboard lands on. 68 holds a COMPONENT and 73 an
   * expression, and guessing at what either renders is how a rule reports a page that is correct.
   * 76 cannot read the attribute and 81 says `false`.
   */
  test("both fixes, and everything this cannot see, stay silent", () => {
    const lines = found().map((issue) => issue.line);
    for (const quiet of [40, 45, 52, 58, 63, 68, 73, 76, 81]) {
