 ...
 *
 * Asked in five places — `@Host` and the two other class decorators reject a class that is not one,
 * `h` decides whether a tag is a component or a host element, and `lazy` decides whether a loaded
 * module has to be wrapped before `render` can call it. Four of the five carried
 * `as unknown as { __isComponent?: boolean }` and the fifth took its argument as `any`, which is the
 * same predicate written by hand each time and narrowing nothing afterwards.
 *
